...
def ensure_collaborator_for_user(user):
    """
    Garante que o usuário tenha um Collaborator vinculado, sem usar o e-mail real
    para formar placeholder. Só usa o e-mail real para localizar/ligar registro existente.
    """
    ...
    # Placeholder neutro (sem e-mail/username reais)
    # Ex.: collab-u6-9f2b1a2c@local.invalid
    uniq = uuid4().hex[:8]
    unique_email = f"collab-u{user.id}-{uniq}@local.invalid"

    # Nome amigável mas sem PII crítica
