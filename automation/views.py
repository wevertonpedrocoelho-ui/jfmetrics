...
AUTO_DEPT_SLUG = "engauto"   # <-- ajuste se o slug do depto for diferente (ex.: "engauto")
AUTO_NS = "engauto"

def _get_user_collaborator(user):
    """Retorna o Collaborator central do usuário para o departamento de Automação."""
...
def get_user_activity_or_404(user, pk):
    """Busca Activity garantindo que pertence ao colaborador do usuário atual (Automação)."""
...
def activity_create(request):
    ...
    return render(request, "automation/activity_form.html", {"form": form})
...
def profile(request):
    return HttpResponse("Perfil (placeholder)")
...
def project_list(request):
    ...
    return render(request, "projects/list.html", {"q": q, "page_obj": page_obj})
...
def eap_view(request):
    specific_qs = SpecificActivity.objects.filter(is_active=True).order_by("order", "name")
    general_qs = (
        GeneralActivity.objects.filter(is_active=True)
        .order_by("order", "name")
        .annotate(num_specifics=Count("specifics", filter=Q(specifics__is_active=True)))
        .prefetch_related(Prefetch("specifics", queryset=specific_qs))
    )
    milestones = (
        Milestone.objects.filter(is_active=True)
        .order_by("order", "name")
        .annotate(num_generals=Count("generals", filter=Q(generals__is_active=True)))
        .prefetch_related(Prefetch("generals", queryset=general_qs))
    )
    return render(request, "eap/index.html", {"milestones": milestones})
...
def reports_entry(request):
    ...
    - se o usuário tiver Collaborator (Automação) -> redireciona para o dashboard dele
    - caso contrário -> mostra lista de colaboradores para escolher
    """
    cid = request.GET.get("c")
    if cid and cid.isdigit():
        return redirect("automation:collaborator_dashboard", pk=int(cid))

    me = _get_user_collaborator(request.user)
    if me:
        return redirect("automation:collaborator_dashboard", pk=me.id)
    ...
    return render(request, "reports/select_collaborator.html", {
        "q": q,
        "page_obj": page_obj,
    })
...
def collaborator_dashboard(request, pk: int):
    ...
    return render(request, "collaborators/dashboard.html", context)
...
def settings(request):
    ...
    return render(request, "configuration/settings.html", {
        "form": profile_form,
        "pwd_form": pwd_form,
    })
...
def realtime(request):
    ...
    return render(request, "realtime/index.html", {"activities": activities})
...
def realtime_fragment(request):
    ...
    activities = (Activity.objects
                  ...
                  .filter(is_active=True, finished_at__isnull=True)
                  .order_by("collaborator__name", "-id"))
    return render(request, "realtime/_list.html", {"activities": activities})
...
def department_report(request):
    ...
    return render(request, "reports/department_dashboard.html", ctx)
...
def department_report_pdf(request):
    ...
    return HttpResponse("PDF do Relatório Geral (implementar)", content_type="text/plain")
...
def catalogs(request):
    ...
    return render(request, "epe/catalogs/index.html", ctx)
